...
#/ Get papers for each category
for CATEGORY in CATEGORIES
    @info "Computing Heap's law for $(CATEGORY)"
    ...
    if nsamples < 9 && factorial(nsamples) > npermutations
        _npermutations = factorial(nsamples)
    else
        _npermutations = copy(npermutations)
    end
    ...
    #~ Save
    if save
        filename = "arxiv-$(CATEGORY)-vocabsize.jld2"
